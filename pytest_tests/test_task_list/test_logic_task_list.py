...
def test_user_can_create_task(author_client, author, form_data):
    ...
    assert new_task.title == form_data['title']
    assert new_task.content == form_data['content']
    assert new_task.author == author


@pytest.mark.django_db
def test_anon_user_cant_create_task(client, form_data):
    url = reverse('tasks:add_task')
...
def test_author_can_edit_task(author_client, task, form_data):
    ...
    assert task.content == form_data['content']


def test_not_author_cant_edit_task(not_author_client, task, form_data):
    url = reverse('tasks:task', args=(task.id,))
